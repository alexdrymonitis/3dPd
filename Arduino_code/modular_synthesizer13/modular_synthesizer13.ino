...
void checkConnections(int pin, int module) {
  ...
  for(int i = 0; i < NUM_OF_MODULES; i++){
    ...
    if(maskedBanana != bananaStates[pin][i]){
      ...
      // the split byte needs the 13bug fix
      if ((maskedBanana & 0x7f) == 13) transferData[localIndex++] = 0;
      else transferData[localIndex++] = 1;
      transferData[localIndex++] = maskedBanana >> 7;
      if ((maskedBanana >> 7) == 13) transferData[localIndex++] = 0;
      else transferData[localIndex++] = 1;
...
void checkSwitches() {
  ...
  for(int i = 0; i < NUM_OF_MODULES; i++){
    ...
    if(maskedSwitch != switchStates[i]){
      ...
      if (i == 13) transferData[localIndex++] = 0;
      else transferData[localIndex++] = 1;
      ...
      if ((maskedSwitch & 0x7f) == 13) transferData[localIndex++] = 0;
      else transferData[localIndex++] = 1;
      transferData[localIndex++] = maskedSwitch >> 7;
      if ((maskedSwitch >> 7) == 13) transferData[localIndex++] = 0;
      else transferData[localIndex++] = 1;
  ...
  for(int masterMux = 0; masterMux < numOfMasterMux; masterMux++){
    ...
    for(int slaveMux = 0; slaveMux < numOfSlaveMux[masterMux]; slaveMux++){
      ...
      if(activeModules[moduleIndex]){
        ...
        if (moduleIndex == 13) transferData[localIndex++] = 0;
        else transferData[localIndex++] = 1;
      ...
      for(int pot = 0; pot < numOfPots[masterMux][slaveMux]; pot++){
        ...
        if(activeModules[moduleIndex]){
          ...
          if ((smoothed & 0x007f) == 13) transferData[localIndex++] = 0;
          else transferData[localIndex++] = 1;
          transferData[localIndex++] = smoothed >> 7;
          if ((smoothed >> 7) == 13) transferData[localIndex++] = 0;
          else transferData[localIndex++] = 1;
        }
...
void sendAnalogReadResolution() {
  ...
  // tell Pd how many more bytes to expect beyond the first index
  // we should expect the generic data secondary index and the analogRead resolution split in two
  transferData[localIndex++] = 5;
...
void setup() {
  ...
  // transferData array
  // since each function writes separately, the maximum size is totalPots * 4
  //(the pot values split in two plus the 13-bug fixes)
  // plus total number of modules * 5 for the data indexes, number of pots, and 13-bug fix
  // plus 1 for the start character
  numOfTotalData = (totalPots * 4) + (NUM_OF_MODULES * 5) + 1;
